...
DEFAULT_SYSTEM_PROMPT = (
     ...
     * Questions about 'epic', 'strategic initiative', 'large feature' → content_type='epic'\n"
    ...
    "   - Examples: 'create a bug report', 'generate documentation page', 'draft meeting notes', 'create sprint', 'generate module', 'create epic'.\n"
...
- 'What epics are in progress?' → rag_search(query='in progress epics', content_type='epic')\n"
...
- 'Create user auth epic' → generate_content(content_type='epic', prompt='User Authentication System')\n\n"
                    "WHEN UNSURE WHICH TOOL:\n"
                    "- If the query is ambiguous or entity/field mapping to Mongo is unclear → prefer rag_search first.\n"
                    "- Question about structured data (counts, filters, group by, breakdown by assignee/state/priority/project/date) → mongo_query.\n"
                    "- Question about content meaning/semantics (find docs, analyze patterns, content search, descriptions) → rag_search.\n"
