...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "3) Use 'generate_content' to CREATE new content. Supports: work_item, page, cycle, module, epic, user_story, feature, project.\n"
   ...
   "   - For complete project design, follow hierarchy: project → epics → modules → features → user_stories → cycles → work_items.\n"
   "   - Call SEQUENTIALLY when generating multiple items.\n"
   "   - End-to-end example: 'Create a complete e-commerce project' → generate_content(project) → generate_content(epic) → generate_content(module) → generate_content(feature) → generate_content(user_story) → generate_content(cycle) → generate_content(work_item).\n"
