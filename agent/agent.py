...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "3) Use 'generate_content' to CREATE new work items, pages, cycles, modules, epics, user stories, features, or projects.\n"
    ...
    "   - REQUIRED: content_type ('work_item', 'page', 'cycle', 'module', 'epic', 'user_story', 'feature', or 'project'), prompt (user's instruction).\n"
    "   - OPTIONAL: template_title, template_content, context.\n"
    "\n"
    "   **END-TO-END PROJECT DESIGN EXAMPLE**:\n"
    "   When asked to design and generate a complete project (e.g., 'Design an e-commerce platform'), follow this hierarchy:\n"
    "   \n"
    "   Step 1: CREATE THE PROJECT - Establish the foundation\n"
    "      generate_content(content_type='project', prompt='E-commerce Platform - Online marketplace for small businesses')\n"
    "   \n"
    "   Step 2: CREATE EPICS (3-5) - High-level strategic initiatives\n"
    "      generate_content(content_type='epic', prompt='User Authentication & Security - Login, registration, OAuth, 2FA')\n"
    "      generate_content(content_type='epic', prompt='Product Catalog Management - Product listings, categories, search')\n"
    "      generate_content(content_type='epic', prompt='Shopping Cart & Checkout - Cart management, payment processing')\n"
    "      generate_content(content_type='epic', prompt='Order Management - Order tracking, fulfillment, returns')\n"
    "   \n"
    "   Step 3: CREATE MODULES (2-3 per epic) - Functional groupings\n"
    "      generate_content(content_type='module', prompt='Authentication Module - User login, registration, password recovery')\n"
    "      generate_content(content_type='module', prompt='Product Search Module - Search engine, filters, sorting')\n"
    "      generate_content(content_type='module', prompt='Payment Processing Module - Payment gateways, transactions')\n"
    "   \n"
    "   Step 4: CREATE FEATURES (2-4 per module) - Detailed specifications\n"
    "      generate_content(content_type='feature', prompt='Social Login - Google and Facebook OAuth integration')\n"
    "      generate_content(content_type='feature', prompt='Advanced Product Search - Full-text search with filters')\n"
    "      generate_content(content_type='feature', prompt='Multi-Payment Gateway - Stripe, PayPal, credit card support')\n"
    "   \n"
    "   Step 5: CREATE USER STORIES (2-3 per feature) - User-centric requirements\n"
    ...
    "      generate_content(content_type='user_story', prompt='As a customer, I want to filter products by price range so I can find items in my budget')\n"
    "      generate_content(content_type='user_story', prompt='As a buyer, I want to save my payment method so I can checkout faster next time')\n"
    "   \n"
    "   Step 6: CREATE CYCLES (2-4) - Development sprints\n"
    "      generate_content(content_type='cycle', prompt='Sprint 1: Authentication Foundation - 2 weeks - Core login/registration')\n"
    "      generate_content(content_type='cycle', prompt='Sprint 2: Product Catalog MVP - 2 weeks - Basic product listing and search')\n"
    "      generate_content(content_type='cycle', prompt='Sprint 3: Shopping Experience - 2 weeks - Cart and checkout flow')\n"
    "   \n"
    "   Step 7: CREATE WORK ITEMS (3-5 per cycle) - Specific tasks\n"
    "      generate_content(content_type='work_item', prompt='Implement OAuth2 callback handler for Google login')\n"
    "      generate_content(content_type='work_item', prompt='Design and implement product card component')\n"
    "      generate_content(content_type='work_item', prompt='Integrate Stripe payment SDK')\n"
    "   \n"
    "   IMPORTANT: Call generate_content SEQUENTIALLY (not in parallel) so each item appears individually.\n"
    "   Acknowledge each generation briefly, then proceed to the next level of the hierarchy.\n"
    ...
    "- generate_content(content_type:str, prompt:str, template_title:str='', template_content:str='', context:dict=None): Generate project artifacts.\n"
    "  REQUIRED: content_type ('work_item'|'page'|'cycle'|'module'|'epic'|'user_story'|'feature'|'project'), prompt (what to generate).\n"
    "  OPTIONAL: template_title, template_content, context.\n"
    "  NOTE: Returns '✅ Content generated' only - full content sent directly to frontend to save tokens.\n"
    "  HIERARCHY: project → epics → modules → features → user_stories → cycles → work_items\n\n"
