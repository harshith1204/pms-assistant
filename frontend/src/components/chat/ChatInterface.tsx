...
export function ChatInterface() {
	...
	const handleWebSocketMessage = useCallback((data: WsEvent) => {
		...
		switch (data.type) {
			...
			case "user_message": {
				setConversationId(getString((data as Record<string, unknown>)["conversation_id"]))
				break;
			}
			...
			case "token": {
				const token = getString((data as Record<string, unknown>)["content"]);
			...
			case "agent_action": {
				const actionMsg: Message = {
					...
					type: "action",
					content: getString((data as Record<string, unknown>)["text"], "Working on it…"),
					timestamp,
				};
			...
			case "agent_result": {
				const resultMsg: Message = {
					...
					type: "result",
					content: getString((data as Record<string, unknown>)["text"], "Done"),
					timestamp,
				};
			...
			// Legacy events kept for backward-compat but hidden from UI to avoid duplication
			case "tool_start": {
				// No-op: ACTION lines replace this
				break;
			}

			case "tool_end": {
				// Optionally keep raw output if toggled; default suppress
				const output = (data as Record<string, unknown>)["output"];
				if (showToolOutputs && output) {
			...
			case "error":
				toast({
					title: "Error",
					description: (data as Record<string, unknown>)["message"] as string || "An error occurred",
					variant: "destructive",
				});
				setIsLoading(false);
				break;
