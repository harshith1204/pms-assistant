...
class Mem0Manager:
    ...
    - Stores memories in Qdrant (vector DB) for semantic retrieval
    - Uses LLM to determine memory relevance and summarization
    - Supports user-specific and agent-specific memory scoping
    
    Configuration via environment variables:
    - MEM0_QDRANT_HOST: Qdrant server host (default: localhost)
    - MEM0_QDRANT_PORT: Qdrant server port (default: 6333)
    - MEM0_QDRANT_PATH: Qdrant local path for embedded mode (optional)
    - MEM0_LLM_PROVIDER: LLM provider (default: groq)
    - MEM0_EMBEDDING_MODEL: Embedding model (default: sentence-transformers/all-MiniLM-L6-v2)
    """
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        qdrant_path = os.getenv("MEM0_QDRANT_PATH")  # For embedded mode
        
        # LLM configuration (using Groq by default to match existing setup)
        llm_provider = os.getenv("MEM0_LLM_PROVIDER", "groq")
        groq_api_key = os.getenv("GROQ_API_KEY")
        groq_model = os.getenv("GROQ_MODEL", "llama-3.3-70b-versatile")
        
        # Embedding configuration
        embedding_model = os.getenv("MEM0_EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2")
        ...
        config = {
            "vector_store": {
                ...
                "config": {
                    ...
                    "embedding_model_dims": 384,  # Dimension for all-MiniLM-L6-v2
                }
            },
            "llm": {
