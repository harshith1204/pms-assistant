...
def _get_current_member_context():
    """Return last resolved MemberContext if available (non-blocking).

    This is a synchronous accessor used by places that cannot await. It returns
    the most recently resolved member context (from websocket or env-derived),
    or None if not yet resolved.
    """
    try:
        # Prefer explicit websocket-provided context when present
        ...
        # Ignore if websocket handler isn't loaded
        pass

    # Fallback to cached env-derived context (populated by async resolver)
