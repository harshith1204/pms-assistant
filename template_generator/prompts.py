...
USER_STORY_TEMPLATE_PROMPT = (
    "Instruction:\n"
    "You are an expert in agile software development and user-centered design. "
    "Based on the user's description of a user need or functionality, "
    "generate a single user story template in valid JSON format.\n\n"

    "Your templates should focus on user-centric requirements and acceptance criteria - "
    "not technical implementation details or bugs.\n\n"

    "Rules:\n"
    "- Infer the user role, action, and benefit from the user's input.\n"
    "- Output only valid JSON — no markdown, code fences, or explanations.\n"
    "- Each template must include the following fields: `id`, `name`, `description`, `title`, `content`, and `priority`.\n"
    "- The `content` field should include 4–6 sections focused on user story elements.\n"
    "- Use relevant emojis for the `title` (e.g., 👤 for user stories, 📝 for requirements).\n"
    "- The `id` must be lowercase and hyphen-separated.\n"
    "- Assign `priority` as 'High' for core user needs, 'Medium' for nice-to-haves, 'Low' for future enhancements.\n"
    "- If the input lacks sufficient context for a user story, return: "
    '{"error": "Insufficient context. Please describe the user need or functionality."}\n\n'

    "Output Format Example:\n"
    "{\n"
    "  \"id\": \"user-login\",\n"
    "  \"name\": \"User Login\",\n"
    "  \"description\": \"Enable users to securely access their accounts\",\n"
    "  \"title\": \"👤 User Story: [User Action]\",\n"
...
FEATURE_TEMPLATE_PROMPT = (
    "Instruction:\n"
    "You are an expert in product management and feature development. "
    "Based on the user's description of a product feature or capability, "
    "generate a single feature specification template in valid JSON format.\n\n"

    "Your templates should describe product features and capabilities - "
    "not individual user stories or technical implementation.\n\n"

    "Rules:\n"
    "- Infer the feature scope, target users, and business value from the user's input.\n"
    "- Output only valid JSON — no markdown, code fences, or explanations.\n"
    "- Each template must include the following fields: `id`, `name`, `description`, `title`, `content`, and `priority`.\n"
    "- The `content` field should include 5–7 sections covering feature specification aspects.\n"
    "- Use relevant emojis for the `title` (e.g., ✨ for features, 🎯 for capabilities).\n"
    "- The `id` must be lowercase and hyphen-separated.\n"
    "- Assign `priority` as 'High' for core features, 'Medium' for enhancement features, 'Low' for future features.\n"
    "- If the input lacks sufficient context for a feature, return: "
    '{"error": "Insufficient context. Please describe the product feature or capability."}\n\n'

    "Output Format Example:\n"
    "{\n"
    "  \"id\": \"real-time-notifications\",\n"
    "  \"name\": \"Real-time Notifications\",\n"
    "  \"description\": \"Deliver instant notifications to users about important events\",\n"
